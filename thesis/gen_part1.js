const {
  ...
  HeadingLevel, PageBreak, BorderStyle, LevelFormat,
...
const MARGIN_LEFT = 1701;   // 30mm (binding)
...
// Helper: normal body paragraph with indent
function p(text, opts = {}) {
  return new Paragraph({
    alignment: opts.align || AlignmentType.JUSTIFIED,
    indent: opts.noIndent ? undefined : { firstLine: 708 }, // 1.25cm indent
...
// Helper: centered paragraph (no indent)
function pc(text, opts = {}) {
  return p(text, { ...opts, align: AlignmentType.CENTER, noIndent: true });
}
...
// Helper: section heading (РОЗДІЛ X)
function heading1(text) {
  return new Paragraph({
    alignment: AlignmentType.CENTER,
    pageBreakBefore: false,
    spacing: { line: 360, lineRule: LineRuleType.AUTO, before: 240, after: 240 },
    indent: { firstLine: 0 },
...
function heading2(text) {
  return new Paragraph({
    ...
    spacing: { line: 360, lineRule: LineRuleType.AUTO, before: 200, after: 120 },
    indent: { firstLine: 0 },
...
if (require.main === module) {
  Packer.toBuffer(doc).then(buf => {
    fs.writeFileSync("D:/apps/dyploma/thesis/thesis_part1.docx", buf);
    console.log("Done: thesis_part1.docx");
